<?php
    ...
    function getBills($tablename="") {
        global $conn;
        if(!empty($tablename)){
            $query = "select  DISTINCT oid,date,name,phone,email,address,total,status from $tablename"; // Change 'products' to your actual table name
            ...
            if ($result) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $rows .= '<td>' . $row['total'] . '</td>';
                    $rows .= '<td>' . $stat . '</td>';
                    $rows .= '<td><a href="invoice-bill.php?oid=' . $uid . '"><i class="fa fa-eye text-primary"></i></a>
                            <a href="editbill.php?oid=' . $uid . '"><i class="fa fa-edit text-primary"></i></a></td>';
