<?php 
    ...
    function placeOrder($name,$phone,$email,$address,$oid,$pname,$price,$quantity,$total,$mrp,$mrptotal,$proid,$discount_total,$totalmrproduct,$discount_per,$date){
        global $conn;
        $query = "insert into orders(name,phone,email,address,oid,pname,price,quantity,total,mrp,mrp_total,p_id,overall_total,overall_mrp_total,discount,date) 
    ...
    //     global $conn;
    //     $query = "select count(distinct(oid)) from orders";
    //     $result = mysqli_query($conn,$query);
